...
Первый запуск скачает Ollama-образ и модель `qwen2.5-coder:14b` (~9 ГБ). Модель загружается автоматически из `docker.env` (`OLLAMA_MODEL`).
...
- **Модель**: `MODEL_TO_USE=OLLAMA | GIGACHAT` (из `docker.env`). По умолчанию Ollama c `qwen2.5-coder:14b`.
...
- **Ollama** — контейнер загружает `qwen2.5-coder:14b` при первом запуске
...
4. **Модель не найдена в Ollama** — при первом запуске docker compose может не успеть загрузить `qwen2.5-coder:14b`. Проверьте:
   ```bash
