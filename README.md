...
Загружает `.xlsx`, через LLM (Ollama или GigaChat) определяет входные ячейки (цена сырья, объёмы и т.д.) и выходные показатели (Debt/EBITDA, выручка и т.д.), запускает сценарии «что-если» с пересчётом формул через `formulas` (in-memory).
...
curl -s -X POST http://localhost:8080/api/v1/invoke-agent \
...
curl -s -X POST http://localhost:8080/api/v1/invoke-agent \
  ...
  -H "x-user-id: $U" \
  -H "x-request-time: 2026-06-08T12:00:00Z" \
  -d '{"message": "Проанализируй model.xlsx со значениями метанола 2025 (450, 500) с шагом 5 и инфляции USD CPI 2025 (0,1 , 0,2) с шагом 0,1. Покажи изменения debt/ebitda, net debt/ebitda (ltm) и icr corr (ltm) 2025 для таких значений"}' \
  --output response.zip && unzip -p response.zip txt_response.txt
...
Ранняя версия открывала книгу только в `data_only=True` для чтения значений, а при `save()` теряла все формулы — `formulas`-движок не мог пересчитать модель.

### Решение: двухкнижечный паттерн + formulas
...
`calculate()` использует `formulas.ExcelModel.calculate(inputs=self._inputs, outputs=...)` — полный пересчёт графа формул **в памяти**, без внешних процессов. Результат кешируется в `self._solution`. При `set_cell()` кеш сбрасывается.

Модель загружается через `formulas.ExcelModel().loads(file).finish()` (~34s при первом запуске) и кешируется `@lru_cache(maxsize=3)` — повторные вызовы для того же файла мгновенны.
...
- **Модель**: `MODEL_TO_USE=OLLAMA | GIGACHAT` (из `docker.env`). По умолчанию Ollama c `qwen2.5:7b`.
...
Подробнее о тестах, валидации вызовов инструментов и формате вывода — [tests/README.md](tests/README.md).

## Требования

- **Docker + Docker Compose**
- **formulas** (pure Python) — in-memory evaluation, не требует внешних программ
- **Ollama** — контейнер загружает `qwen2.5:7b` при первом запуске
...
3. **Модель не найдена в Ollama** — при первом запуске docker compose может не успеть загрузить `qwen2.5:7b`. Проверьте:
   ```bash
