...
U="$USER"  # или любая строка до 8 символов
...
- **LibreOffice Calc** — установлен в контейнере (`apt-get instal libreoffice-calc`)
...
4. **Модель не найдена в Ollama** — при первом запуске docker compose может не успеть загрузить `qwen2.5:7b`. Проверьте:
   ```bash
